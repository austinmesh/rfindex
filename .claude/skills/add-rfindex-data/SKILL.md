...
description: Use when adding a new device, antenna, or a manufacturer/supplier/feature it depends on to RF Index — builds schema-valid JSON in data/ from a product URL. Triggers on "add a device", "add an antenna", "new device/antenna", "import this product", "submit antenna test data / VNA sweep / .s1p".
---

# Add RF Index Data

Author a new `data/` record — a device, an antenna, or a device relation
(manufacturer / supplier / feature) — that passes `pnpm validate` on the
first real try. You take a product URL and a few answers; this skill turns
them into correct JSON with the right enums, relations, referral links, and
image handling.
...
bands/modulations/radios/antenna_connectors collections.
...
- Convertible enums (full lists in the schema): `supported_firmware`
  ["Meshtastic","MeshCore"]; `category` ["DIY","Complete","Solar","Standalone"];
  `lora_frequencies` use spaced form `"915 MHz"` (not `915MHz`);
  `specifications.max_tx_power_dbm` is **dBm not watts** (0.5 W=27, 1 W=30,
...
## Relations (devices only — validate does NOT catch these)

The schema types `manufacturer`, `purchase_urls[].supplier`, and `features[]`
as plain strings, so `pnpm validate` accepts typos. Reuse **exact**
(case-sensitive) matches. Use the helper script to list or check values:

```bash
# list every existing manufacturer, supplier, and device feature value
node .claude/skills/add-rfindex-data/list-relations.mjs

# check one value — reports EXACT / near-match (typo) / new, per relation type
...
- Feature missing → **stop and confirm explicitly.** `features` is a
  ...
  a genuinely new, buyer-facing distinction. (`Solar` = built-in panel;
...
Fix every `FAIL:` (schema) and `INVALID JSON:` (syntax — usually a trailing
comma) until it prints "All files passed." Validation only checks shape, so
also re-confirm relations and referral params by eye before finishing.
...
| Typo'd manufacturer/supplier/feature | validate won't catch it — check reference dirs / device features by hand |
| Power as watts | use `max_tx_power_dbm` (0.5W=27, 1W=30, 2W=33) |
| LoRa radio in `features` | it belongs in `specifications.lora_radio` |
| New `features` value | curated short list — confirm before coining one |
